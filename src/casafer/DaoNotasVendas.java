...
public ArrayList todosPedidos(String dataInicio,String dataFim) throws ClassNotFoundException {
        conexao.Conectar();
          
        ArrayList<Pedidos> listaPedidos = new ArrayList();
        try {
            String query = "SELECT * FROM pedidosvenda where Ped_Data BETWEEN date("+dataInicio+") AND  date("+dataFim+") AND Ped_Situacao = 'F'";
            PreparedStatement pst;
            ResultSet rs;
            pst = conexao.con.prepareStatement(query);
            rs = pst.executeQuery();
...
public ArrayList todasNotasEntrada(String dataInicio,String dataFim) throws ClassNotFoundException {
        conexao.Conectar();
        
        ArrayList<NotasCompras> listaNotas = new ArrayList();
        try {
            String query = "SELECT n.For_Codigo,f.For_RazaoSocial,n.NF_Numero,n.NF_DataEmissao,n.NF_DataLcto,n.NF_VlrMercadorias,n.NF_VlrNota,n.NF_VlrFrete,n.NF_ChaveNFe FROM notascompra n INNER JOIN fornecedores f ON f.For_Codigo = n.For_Codigo WHERE n.NF_DataLcto BETWEEN date("+dataInicio+") AND  date("+dataFim+")";
            PreparedStatement pst;
            ResultSet rs;
            pst = conexao.con.prepareStatement(query);
            rs = pst.executeQuery();
            while (rs.next()) {
                ...
                NotasCompras nota = new NotasCompras();
                nota.setFor_Codigo(rs.getInt("For_Codigo"));
                nota.setFor_RazaoSocial(rs.getString("For_RazaoSocial"));
                nota.setNF_Numero(rs.getInt("NF_Numero"));
                nota.setNF_DataEmissao(rs.getString("NF_DataEmissao"));
                nota.setNF_DataLcto(rs.getString("NF_DataLcto"));
                nota.setNF_VlrMercadorias(rs.getDouble("NF_VlrMercadorias"));
                nota.setNF_VlrNota(rs.getDouble("NF_VlrNota"));
                nota.setNF_VlrFrete(rs.getDouble("NF_VlrFrete"));
                nota.setNF_ChaveNFe(rs.getString("NF_ChaveNFe"));
                listaNotas.add(nota);
            }
        } catch (SQLException e) {
            System.out.println("Erro na Conexão com o Banco " + e);
